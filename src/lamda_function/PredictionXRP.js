...
exports.handler = async (event) => {
    ...
    // Check if the action is XRPPredictionData and the coin is XRP
    if (action === 'XRPPredictionData' && coin === 'XRP') {
        try {
            ...
            // Update the endpoint names to the ones corresponding to XRP
            const priceTypes = {
                'Low': 'LowXRP-endpoint',
                'High': 'HighXRP-Endpoint',
                'Open': 'Open-XRP-Endpoint',
                'Close': 'Close-XRP-endpoint'
            };

            console.log('Retrieving latest data points...');
            const latestDataPoints = await getLatestDataPoints('Cryto_Data_1', 'XRP');
        ...
        console.log('The event did not contain action XRPPredictionData and coin XRP');
